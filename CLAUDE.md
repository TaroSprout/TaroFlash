...
- **Never verify visually in the browser.** Don't open Chrome, curl the dev server, or screenshot a page to check that a change "looks right" — I always confirm visually myself and will give you that feedback directly. Your visual read is worse than mine, so it adds false confidence rather than verification. Driving the browser to _debug_ something genuinely broken is fine and encouraged; the ban is on verification, not on debugging. When the symptom itself is only observable running — visual, animation, CSS-timing, layout — capture the actual broken behavior in the browser (e.g. a computed-style trace over time) before dispatching a fix; reasoning from source alone about a running-only symptom is a guess, not a diagnosis.
- **A diagnostic ask, or a ticket-cutting ask, stops there.** When I ask what's wrong, why something
  broke, or to cut a ticket for it, that's the whole ask — no source edit, no format/type-check pass,
  no test run, no subagent dispatch, even when the fix is small and obvious. Name the fix in your
  answer, or write it into the ticket; building it waits for a separate ask to build it.
...
- **No one writes a code comment except the `comment-author` agent.** Not a builder, not an ad hoc
  edit — everyone else produces comment-free code. Exempt (not comments): a machine directive
  (`eslint-`, `@ts-`/`@ts-expect-error`, `istanbul ignore`, `c8 ignore`), a `[K:gap: …]` marker, and
  `COPY-TBD`. Shape and gates: [`comment-authoring`](.claude/knowledge/comment-authoring.md) — read by
  name, never auto-loaded.
...
| Any `.ts` / `.vue`                       | [`code-style`](.claude/rules/code-style.md), [`FE-formatting`](.claude/rules/FE-formatting.md), [`animations`](.claude/rules/animations.md), [`safari-gotchas`](.claude/rules/safari-gotchas.md) — comments are the golden rule above, not this row |
