...
> [!HAZARD] [K:dock-edge-inset-follows-flush] **The device's bottom-edge inset is earned by sitting flush on the screen edge, not by being on a touch device — and the allowance never gets a floor under that inset, because a reported `0px` is the browser saying the strip is already covered.**
...
> `0px` on every desktop browser, and it resolves to `0px` on mobile Safari
> too while the bottom toolbar is up, because that toolbar is what sits over
> the home-indicator strip; it only grows to the real inset once the toolbar
> collapses on scroll. The viewport-gap signal doesn't catch that toolbar —
> Safari shrinks `window.innerHeight` along with the visual viewport, so the
> gap stays zero and the dock still counts as uncovered. That leaves a
> desktop window at phone width and a phone with its toolbar up reporting
> exactly the same thing, so any floor written to give the desktop case a
> visible allowance — `max(1rem, calc(0.5rem + env(safe-area-inset-bottom)))`
> — fires on the phone as well and lifts the bar off the toolbar it should
> be resting on. The allowance stays
> `calc(0.5rem + env(safe-area-inset-bottom))`: worth nothing wherever the
> browser reports nothing, which is exactly where nothing is owed.
