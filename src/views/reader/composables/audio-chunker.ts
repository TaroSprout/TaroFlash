...
// whole audiobook) can't be transcribed in one call. We first transcode the original
// to a compact mono MP3 (speech needs no stereo/hi-fi and Whisper downsamples to
// 16 kHz anyway) — this is the playback asset, and for a short file it's also the
// single transcription input; for a long file we then slice that MP3 into ordered,
// overlapping windows the worker transcribes one per invocation and stitches back by
// `offset`.
//
...
export async function chunkAudio(
  ...
  const input_data = await fetchFile(file)
  await ffmpeg.writeFile(inputName, input_data)
...
async function transcode(
  ...
  const onLog = ({ message }: { message: string }) => {
    // Matches ffmpeg's stderr Duration line (e.g. "Duration: 00:40:12.34").
