    ...
    <slot>
      <template v-for="option in options" :key="option.value">
        ...
        >
          <div
            aria-hidden="true"
            class="pointer-events-none absolute inset-0 hidden bgx-diagonal-stripes bgx-color-[var(--color-raised-pattern)] motion-rich:bgx-slide group-hover/option:block group-data-[tapping=true]/option:block"
          ></div>
          <ui-icon
            v-if="option.icon"
            :src="option.icon"
