...
import { useCreateLessonMutation, EdgeFunctionError } from '@/api/lessons'
...
async function onSubmit() {
  ...
  try {
    const lesson = await create.mutateAsync({ title: title.value.trim(), file: file.value })
    close(lesson)
  } catch (error) {
    ...
    <div data-testid="upload-lesson__body" class="flex flex-col gap-5 p-6">
      ...
      <div data-testid="upload-lesson__actions" class="flex gap-3">
        ...
        >
          {{
            is_uploading
              ? t('audio-reader.upload.uploading-button')
