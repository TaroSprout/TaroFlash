...
import Controls from '@/views/reader/controls.vue'
...
describe('Controls', () => {
  ...
  describe('fires on pointerup, not click', () => {
    test('a click on toggle does nothing', async () => {
    ...
    test('toggle calls player.play() on pointerup when paused, and emits ui.press', async () => {
      const player = makePlayer({ is_playing: ref(false) })
      const wrapper = mountControls({ player })

      await wrapper.find('[data-testid="paged-controls__toggle"]').trigger('pointerup')
    ...
    test('toggle calls player.pause() on pointerup when playing, and emits dialog.open', async () => {
    ...
    test('skip-forward calls player.skip(10) on pointerup, not click', async () => {
      ...
      await wrapper.find('[data-testid="paged-controls__skip-forward"]').trigger('pointerup')
      expect(player.skip).toHaveBeenCalledWith(10)
      expect(mockEmitSfx).toHaveBeenCalledWith('ui.press')
    })

    test('skip-back calls player.skip(-10) on pointerup', async () => {
      const player = makePlayer()
      const wrapper = mountControls({ player })

      await wrapper.find('[data-testid="paged-controls__skip-back"]').trigger('pointerup')

      expect(player.skip).toHaveBeenCalledWith(-10)
    })

    test('settings button emits open-settings on pointerup, not click', async () => {
      const wrapper = mountControls()

      await wrapper.find('[data-testid="paged-controls__settings"]').trigger('click')
      expect(wrapper.emitted('open-settings')).toBeUndefined()

      await wrapper.find('[data-testid="paged-controls__settings"]').trigger('pointerup')
