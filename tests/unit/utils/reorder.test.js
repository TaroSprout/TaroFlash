...
describe('resolveReorderAnchor', () => {
  // dropping after 2nd row resolves to { anchor_id: that row, side: 'after' }
  test('dropping after the 2nd row (to=2) picks the row before as after-anchor', () => {
    const without = [c(1), c(2), c(3)]
    expect(resolveReorderAnchor(without, 2)).toEqual({ anchor_id: 2, side: 'after' })
  })

  // dropping at top (to=0) resolves to nearest right row with side='before'
  test('dropping at top (to=0) walks right and picks the first row as before-anchor', () => {
    const without = [c(1), c(2)]
    expect(resolveReorderAnchor(without, 0)).toEqual({ anchor_id: 1, side: 'before' })
  })

  // skips temp rows (id<=0) when walking outward
  test('skips temp rows (id <= 0) and walks to the nearest persisted neighbour', () => {
  ...
  // returns null when no persisted neighbour exists
  test('returns null when no persisted neighbour exists (all temps or empty list)', () => {
  ...
  // deck-reorder caller: dropping at the very first index
  test('dropping at the very first index (to=0) of a multi-row list', () => {
    const without = [c(100), c(200), c(300)]
    expect(resolveReorderAnchor(without, 0)).toEqual({ anchor_id: 100, side: 'before' })
  })

  // deck-reorder caller: dropping at the very last index
  test('dropping at the very last index (to=length) of a multi-row list', () => {
    const without = [c(100), c(200), c(300)]
  ...
  // an anchor row missing an id resolves to a no-op/null, not a throw
  test('a row with a missing id is treated as unresolvable and skipped without throwing', () => {
    const without = [{}, {}]
