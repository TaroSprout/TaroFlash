...
vi.mock('@/views/reader/composables/audio-player', () => ({ useAudioPlayer: audioPlayerMock }))
vi.mock('@/views/reader/composables/transcript-sync', () => ({
  useTranscriptSync: transcriptSyncMock
}))
vi.mock('@/views/reader/composables/reader-prefs', () => ({ useReaderPrefs: readerPrefsMock }))

// The transcript utils are pure and left real, so paragraph shaping is tested end to end.
import { useLessonReader } from '@/views/reader/composables/lesson-reader'
