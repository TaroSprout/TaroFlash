...
3. **Grep before writing.** CLAUDE.md, `.claude/rules/*`, `.claude/skills/*`, `MEMORY.md`. Existing
   coverage means sharpen or relocate, never add a second copy.
4. **Route** by the table in `self-heal.md`. Fast lane (personal taste) → write the memory + its
   `MEMORY.md` line and you're done, no PR. Durable lane → step 5.
...
- Don't touch tests, don't touch product code. This skill edits docs, rules, skills, memory, and
  hooks only.
