...
- **Read-only.** Your tools are `Read, Grep, Glob, Bash, Skill`. You never edit a file, never commit,
  ...
  routes every fix.
- **One concern.** You judge only the family your `--concern` names; `review-work` scopes you to it.
  A violation of a different rule family is another reviewer's, not yours.
