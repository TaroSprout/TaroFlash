...
- Every trap is **echoed** in the directory it bites, as a comment in that site's own shape that
  passes every [`comment-authoring`](../knowledge/comment-authoring.md) gate on its own — competent-stranger
  ...
  [`comment-authoring → Pointers`](../knowledge/comment-authoring.md#pointers):
