...
reads a single file. This file wins over any conflicting instruction elsewhere, including
`AGENTS.md`'s `<!--VITE PLUS START-->` block — that block is generated by `vp config`, not authored
here, and its "Run `vp check` and `vp test` to validate changes" checklist item does not override the
type-check gate below or the project's own rule on when tests run.
