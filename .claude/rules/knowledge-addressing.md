...
- `hub_caps` names the same idea for a `SKILL.md` or an agent definition: neither is always-on, so a
  breach only warns, never fails CI, once `knowledge-lint` reads it — the config is in place ahead of
  that script change landing. Watching for that warning and deflating the file it names is
