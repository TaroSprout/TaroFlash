-- =============================================================================
-- get_study_session_cards: interleave new/review cards, order review by due
-- =============================================================================
--
-- Two behavior changes from the original queue builder:
--   1. Review cards are now ordered by `due ASC` (most overdue first) instead
--      of `rank`, so if the daily cap truncates the due set, the cards most
--      at risk of being forgotten are the ones that make the cut.
--   2. New and review cards are interleaved throughout the queue instead of
--      "all new, then all review" — spreads new-card cognitive load across
--      the session instead of front-loading it.
--
-- Same signature as the original (bigint, boolean) → CREATE OR REPLACE is
-- safe here; only the query body inside changes, not the return shape.
-- =============================================================================
