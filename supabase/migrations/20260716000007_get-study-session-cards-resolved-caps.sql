-- get_study_session_cards was still reading daily caps straight off
-- decks.study_config, which no longer carries max_reviews_per_day /
-- max_new_per_day (moved to review_pacing_presets + deck_review_pacing in
-- 20260716000004). Point the cap lookup at the same resolution rule
-- get_member_decks uses instead. Same signature -> CREATE OR REPLACE is
-- safe, only the query body changes.
